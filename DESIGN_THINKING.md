...
### 5.5 경기 결과 자동 수집 플로우 (구현 완료)
1. 관리자 로그인 확인
2. 경기 결과 자동 수집 페이지 접근 (`/admin/crawl-results`)
3. "경기 결과 수집 시작" 버튼 클릭
4. 시스템이 joinkfa.com에 접속하여 경기 결과 크롤링
   - Playwright 브라우저 자동화
   - Network API 응답 가로채기 (`getMatchList.do`, `getInitData` 등)
   - 고정 대회 ID 사용 (U11, U12)
   - 경기 결과 데이터 추출 (팀명, 점수, 날짜, 시간, 경기번호 등)
5. 크롤링한 경기 결과를 DB의 기존 경기와 매칭
   - 경기번호 우선 매칭 (가장 정확)
   - 팀명+날짜+시간 기반 매칭 (경기번호 없을 때)
   - 연령대 확인
6. 매칭된 경기 결과를 DB에 업데이트
   - 점수 업데이트 (home_score, away_score)
   - 상태 변경 (status = '종료')
   - 경기번호 업데이트 (크롤링한 경기번호가 DB에 없거나 다른 경우)
7. 결과 요약 표시
   - 수집된 경기 수
   - 업데이트된 경기 수
   - 실패한 경기 수 및 목록
...
#### Phase 9: 경기 결과 자동 수집 기능 재구현 ✅ (2026-02-01)
- [x] joinkfa.com 경기 결과 크롤링 로직 구현
  - Playwright 기반 브라우저 자동화
  - Network API 응답 가로채기 방식 (`getMatchList.do`, `getInitData` 등)
...
- [x] 경기 결과 매칭 로직 구현
  ...
  - 경기번호 자동 업데이트 기능
  - 매칭 실패 원인 상세 기록
- [x] 크롤링 API 라우트 구현 (`/api/crawl/results`)
  - 관리자 인증 확인
  - 크롤링 실행 → 매칭 → DB 업데이트 전체 플로우
  - 통계 및 에러 정보 반환
- [x] 관리자 UI 구현 (`/admin/crawl-results`)
  - 크롤링 실행 버튼
  - 진행 상황 표시
...
├── /api/crawl/results (경기 결과 크롤링 API)
└── /admin (관리자 대시보드 - 관리자 전용)
    └── /admin/matches
        ├── /admin/matches (경기 관리 목록)
...
│   └── AgeGroupTabs.tsx (U11/U12 탭)
├── Crawl/
│   └── CrawlResultsForm.tsx (경기 결과 크롤링 실행 폼)
└── auth/
    ├── LoginForm.tsx (로그인 폼)
    ├── LogoutButton.tsx (로그아웃 버튼)
    └── ProtectedRoute.tsx (보호된 라우트)
