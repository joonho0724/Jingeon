...
-- teams/matches를 현재 환경에 맞게 강제로 리셋하기 위한 복구용 스크립트입니다.
-- 운영 환경에서 새 데이터베이스를 만들 때 다시 실행하면 안 됩니다.

BEGIN;
-- ============================================
-- teams 테이블 전체 삭제 후, 00_docs/team-names.txt 기반으로 재삽입
-- 생성일: 2026-02-05T06:43:12.170Z
-- 목적:
-- - teams: U11(63), U12(64) = 총 127개를 파일 내용으로 재구성
-- - matches/fair_play_points가 teams를 참조 중이면 삭제가 실패하므로, 먼저 참조를 NULL로 해제
--
-- ⚠️ 중요:
-- - 이 SQL을 실행하면 기존 team id가 모두 바뀝니다.
-- - 따라서 기존 matches는 home_team_id/away_team_id가 NULL이 됩니다.
--   (경기 데이터를 유지하면서 팀 연결까지 복구하려면 별도의 매핑/업데이트가 필요)
-- ============================================

-- ============================================
-- ⚠️ 핵심: matches의 age_group 자동 설정 트리거가 home_team_id/away_team_id 변경 시
-- NEW.age_group을 teams에서 다시 조회하여 세팅합니다.
-- team_id를 NULL로 만들면 NEW.home_team_id가 NULL이므로 NEW.age_group도 NULL이 되어
-- matches.age_group NOT NULL 제약(23502)이 즉시 발생합니다.
-- 따라서 이 마이그레이션 동안만 트리거를 잠시 비활성화하고, 작업 후 다시 활성화합니다.
-- ============================================
ALTER TABLE matches DISABLE TRIGGER trigger_set_match_age_group;

-- ============================================
-- 1) 기존 teams 참조 해제 (경기/페어플레이 데이터는 유지)
-- ============================================
-- 이제 team_id를 NULL로 해제 (age_group은 이미 NULL이 아님)
UPDATE matches
SET
  home_team_id = NULL
WHERE home_team_id IN (SELECT id FROM teams WHERE age_group IN ('U11', 'U12'));

UPDATE matches
SET
  away_team_id = NULL
WHERE away_team_id IN (SELECT id FROM teams WHERE age_group IN ('U11', 'U12'));
...
-- 작업 완료 후 트리거 재활성화
ALTER TABLE matches ENABLE TRIGGER trigger_set_match_age_group;
